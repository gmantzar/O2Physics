...
struct femtoDreamTripletTaskTrackTrackCascade {
  ...
  aod::femtodreamcollision::BitMaskType MaskBit = -1; // RAFFA: Georgios uses 1. Why this difference?
  ...
  // Partition for selected particles 
  // RAFFA: adapted from G but without eta!!!
  ...
  bool checkChildCuts(PartType posChild, PartType negChild, PartType bachChild)
  {
    if (((posChild.cut() & ChildPos_CutBit) == ChildPos_CutBit &&
         ...
         posChild.pt() > ChildPos_MinpT &&
         negChild.pt() > ChildNeg_MinpT &&
         bachChild.pt() > ChildBach_MinpT) {
      return true;
    } else {
      return false;
    }
  ...
  void doSameEvent(PartitionType groupSelectedTracks, PartitionType groupSelectedCascades, PartType parts, float magFieldTesla, int multCol, float centCol)
  {
    ...
    /// Histograming V0s
    for (auto& V0 : groupSelectedCascades) {
      const auto& posChild = parts.iteratorAt(V0.index() - 3);
      const auto& negChild = parts.iteratorAt(V0.index() - 2);
      const auto& bachChild = parts.iteratorAt(V0.index() - 1);
      ...
      if (checkChildCuts(posChild, negChild, bachChild)) {
        particleHistoSelectedCascades.fillQA<isMC, false>(V0, aod::femtodreamparticle::kPt, multCol, centCol);
    ...
    for (auto& V0 : groupSelectedCascades) {
      ...
      for (auto& [T1, T2] : combinations(CombinationsStrictlyUpperIndexPolicy(groupSelectedTracks, groupSelectedTracks))) {
        ...
        if (ConfIsCPR.value) {
          ...
          if (pairCloseRejectionTrackCascadeSE.isClosePair(T1, V0, parts, magFieldTesla, Q3)) {
            continue;
          }
          if (pairCloseRejectionTrackCascadeSE.isClosePair(T2, V0, parts, magFieldTesla, Q3)) {
            continue;
          }
        }
        ...
        if (!pairCleanerTrackCascade.isCleanPair(T2, V0, parts)) {
          continue;
        }
        if (!pairCleanerTrackCascade.isCleanPair(T1, V0, parts)) {
          continue;
        }
        ...
        ThreeBodyQARegistry.fill(HIST("TripletTaskQA/hMinvSE_Cascade"), Q3, V0.mLambda());
        ThreeBodyQARegistry.fill(HIST("TripletTaskQA/hMinvSE_LambdaChild"), Q3, V0.mAntiLambda());
  ...
  void doMixedEvent(PartitionType groupPartsOne, PartitionType groupPartsTwo, PartitionType groupPartsThree, PartType parts, float magFieldTesla, int multCol)
  {
    for (auto& [T1, T2, V0] : combinations(CombinationsFullIndexPolicy(groupPartsOne, groupPartsTwo, groupPartsThree))) {
      const auto& posChild = parts.iteratorAt(V0.globalIndex() - 3);
      const auto& negChild = parts.iteratorAt(V0.globalIndex() - 2);
      const auto& bachChild = parts.iteratorAt(V0.globalIndex() - 1);
      ...
      if (ConfIsCPR.value) {
        ...
        if (pairCloseRejectionTrackCascadeME.isClosePair(T1, V0, parts, magFieldTesla, Q3)) {
          continue;
        }
        if (pairCloseRejectionTrackCascadeME.isClosePair(T2, V0, parts, magFieldTesla, Q3)) {
          continue;
        }
      }

      // fill inv Mass as a function of Q3 for purity fits
      ThreeBodyQARegistry.fill(HIST("TripletTaskQA/hMinvME_Cascade"), Q3, V0.mLambda());
      ThreeBodyQARegistry.fill(HIST("TripletTaskQA/hMinvME_LambdaChild"), Q3, V0.mAntiLambda());
